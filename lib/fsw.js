...
class Fsw extends Emitter {
  constructor(opts) {
    ...
    this.connectToFsw = () => new Promise((resolve, reject) => {
      this._conn = new esl.Connection(this.address, this.port, this.secret, this.localAddress);
    ...
    this.queryProfile = () => new Promise(async(resolve, reject) => {
      ...
      this.logger.info(`connected to Freeswitch at ${this.address}:${this.profile}, it reports handling SIP on ${this.sipAddress}:${this.sipPort}`);
      resolve();
    });
  ...
  connection_gone(reason) {
    ...
    if (reason === 'authentication_failed') {
      this.logger.info('not reattempting connection due to auth failure: update config file with correct secret and retry');
      return;
    }

    if (reason === 'acl prevented connection') {
      this.logger.info('not reattempting connection due to ACL configuration on Freeswitch: update Freeswitch ACL conf and retry');
      return;
    }
    ...
    if (this.max_attempts && this.attemps >= this.max_attempts) {
      this.retry_timer = null;
      this.logger.info(`Fsw#connection_gone: Couldn't get drachtio connection after ${this.max_attempts} attempts.`);
      return;
    }
    ...
    this.retry_timer = setTimeout(() => {
      ...
      if (this.connect_timeout && this.retry_totaltime >= this.connect_timeout) {
        this.retry_timer = null;
        this.logger.info(`Fsw#connection_gone: Couldn't get Freeswitch connection after ${this.retry_totaltime} ms.`);
        return;
      }
  ...
  _onHeartbeat(evt) {
    ...
    this.logger.info(`${this.id}: sessions (max/current/avail): ${this.maxSessions}/${this.currentSessions}/${this.idleSessions}, cpu idle: ${this.cpuIdle}`);
  }
  ...
  _onError(err) {
    this.logger.error(`${err}: _onError: `);
    this.emit('error', err);
    this.initialize_retry_vars();
  ...
  _onReady() {
    this.logger.info(`${this.id}: connected and ready`);
  }
